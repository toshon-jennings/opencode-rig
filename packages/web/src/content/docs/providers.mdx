...
#### Claude CLI external agent

OpenCode Rig can also detect an authenticated official `claude` CLI and expose it as the separate
**Claude CLI** provider. This runs the installed CLI in non-persistent, no-tools plan mode; Rig does
not read or copy Claude's OAuth credential. Because this is an external agent rather than a normal
model API, OpenCode tools are not passed into it.
...
1. Head over to the [Deep Infra dashboard](https://deepinfra.com/dash), create an account, and generate an API key or scoped JWT.
...
3. Enter your Deep Infra API key or scoped JWT. Scoped JWTs are sent as bearer tokens in the same
   field and can restrict access by model, endpoint, or other DeepInfra policy claims.
...
If `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `GITHUB_TOKEN` is set, or the Copilot CLI/`gh` CLI already
has a usable login, OpenCode Rig detects it automatically and leaves the token in its owning source.
The device login below remains available when no external login is found.
...
#### Codex CLI external agent

OpenCode Rig detects `codex login` and exposes a separate **Codex CLI** provider. It starts the
official local app-server over stdio, using read-only sandboxing, no approvals, and ephemeral
threads. Rig never reads or copies `~/.codex/auth.json`. This external-agent provider returns text
and does not receive OpenCode's tool registry.

---

### Command Code Provider API

OpenCode Rig reuses `COMMAND_CODE_API_KEY` or the API key owned by an authenticated `commandcode`
CLI. Models are discovered from Command Code's provider endpoint, and the credential is resolved at
request time rather than copied into Rig's credential database.

Command Code's Provider plan (or higher) is required for inference through this API. A CLI login on
a Go plan can list models but the provider endpoint rejects inference until the account is upgraded.
